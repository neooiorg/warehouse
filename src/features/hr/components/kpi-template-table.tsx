...
function KpiSheet({ kpi }: { kpi?: KpiTemplate }) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent>
        <SheetHeader>
          <SheetTitle>{kpi ? 'Sửa KPI' : 'Thêm KPI'}</SheetTitle>
        </SheetHeader>
    ...
    <div className='space-y-4'>
                ...
                .map((k) => (
                  ...
                  >
                    <div className='space-y-1'>
                      ...
                      {k.formula && (
                        <p className='font-mono text-xs text-muted-foreground'>{k.formula}</p>
                      )}
                      <div className='flex gap-2'>
                        {k.target != null && (
                          <Badge variant='outline' className='text-xs'>
                            Mục tiêu: {k.target} {k.unit}
                          </Badge>
                        )}
      ...
      {data.length === 0 && (
        <p className='text-sm text-muted-foreground'>
          Chưa có KPI nào. Thêm KPI để theo dõi hiệu quả nhân sự.
        </p>
      )}
    </div>
  );
