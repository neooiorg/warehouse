...
export function StorageOptimizeClient() {
  ...
  const [searchResults, setSearchResults] = useState<Awaited<ReturnType<typeof searchLotHistory>> | null>(null);
  ...
  return (
    <div className='space-y-6'>
      ...
      <Card>
        <CardHeader><CardTitle className='text-sm'>Đề xuất đảo vị trí lưu trữ</CardTitle></CardHeader>
        <CardContent>
          {!warehouseId && <p className='text-sm text-muted-foreground'>Chọn kho để xem đề xuất.</p>}
          {warehouseId && isLoading && <p className='text-sm text-muted-foreground'>Đang phân tích...</p>}
          {warehouseId && !isLoading && suggestions.length === 0 && (
            <p className='text-sm text-muted-foreground'>Không có đề xuất nào. Tất cả hàng hóa đang ở vị trí tối ưu.</p>
          )}
              ...
              <div key={s.lotId} className='flex items-start justify-between rounded-md border p-3'>
                <div className='space-y-1'>
                  <div className='flex items-center gap-2'>
                    ...
                    <Badge variant={priorityColor[s.priority] as any} className='text-xs'>{s.priority === 'high' ? 'Cao' : s.priority === 'medium' ? 'Trung bình' : 'Thấp'}</Badge>
                  </div>
                  <p className='text-xs text-muted-foreground'>Vị trí hiện tại: {s.currentLocationCode ?? 'N/A'} (cách dock {s.currentDistanceToDock ?? '?'} m)</p>
      ...
      {/* Lot history search */}
      <Card>
        <CardHeader><CardTitle className='text-sm'>Tìm kiếm lịch sử kiện hàng</CardTitle></CardHeader>
        <CardContent>
          <form onSubmit={handleSearch} className='flex gap-3'>
            <input
              className='flex-1 rounded-md border px-3 py-2 text-sm'
              placeholder='Nhập mã lot, SKU hoặc tên hàng...'
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
            <Button type='submit' disabled={searching}>
              {searching ? <Icons.spinner className='h-4 w-4 animate-spin' /> : <Icons.search className='h-4 w-4' />}
            </Button>
          </form>
          {searchResults !== null && (
              ...
              <table className='w-full text-sm'>
                ...
                <tbody>
                  {searchResults.length === 0 && (
                    <tr><td colSpan={8} className='px-3 py-4 text-center text-muted-foreground'>Không tìm thấy kết quả.</td></tr>
                  )}
                  {searchResults.map((r) => (
