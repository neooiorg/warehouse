...
export function TransactionTimeline({ transactions }: { transactions: TransactionWithDetails[] }) {
  if (transactions.length === 0) {
    return (
      <p className='text-muted-foreground py-8 text-center text-sm'>No movement history found.</p>
    );
  }
    ...
    <ol className='space-y-3'>
      {transactions.map((tx) => (
        <li key={tx.id} className='flex items-start gap-3 rounded-lg border p-3'>
          <Badge variant={TYPE_VARIANT[tx.type]} className='mt-0.5 shrink-0 capitalize'>
            {tx.type}
          </Badge>
          <div className='min-w-0 flex-1 space-y-1'>
            <div className='flex flex-wrap items-center gap-x-2 text-sm'>
              ...
              <span className='text-muted-foreground'>qty {tx.qty}</span>
            </div>
